export default function LecturerSidebar({ activePage, onNavigate, isOpen, setIsOpen }) {
  const menuItems = [
  ...
  return (
    <>
      {/* Mobile Dark Overlay - Only appears on phones when sidebar is open */}
      {isOpen && (
        <div 
      ...
      {/* The Sidebar (Fixed on mobile, Relative/Permanent on laptop) */}
      <aside 
        ...
        <nav className="flex-1 space-y-1 px-4 py-4">
          {menuItems.map((item) => (
            <button
              ...
              onClick={() => {
                onNavigate(item.id);
                setIsOpen(false); // Auto-closes on mobile when you tap a link
              }}
